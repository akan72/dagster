...
def _build_graph_node_dict(
    ...
    for node_def in node_defs:
        uses_of_solid = name_to_aliases.get(node_def.name, {node_def.name})

        for alias in uses_of_solid:
            solid_instance = alias_to_node_invocation.get(alias)
            ...
            if isinstance(node_def, GraphDefinition):
                ...
                check.failed(f"Unexpected solid_def type {node_def}")
            nodes.append(node)

    return {ps.name: ps for ps in nodes}
